...
def initialize_datadir(dirname, n):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "onex.conf"), 'w') as f:
        f.write("regtest=1\n")
        f.write("rpcuser=rt\n")
...
def wait_for_bitcoind_start(process, url, i):
    '''
    Wait for onexd to start. This means that RPC is accessible and fully initialized.
    Raise an exception if onexd exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception('onexd exited with status %i during initialization' % process.returncode)
        try:
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None):
    """
    Start a onexd and return RPC connection to it
    """
    datadir = os.path.join(dirname, "node"+str(i))
    if binary is None:
        binary = os.getenv("ONEXD", "onexd")
    ...
    bitcoind_processes[i] = subprocess.Popen(args)
    if os.getenv("PYTHON_DEBUG", ""):
        print "start_node: onexd started, waiting for RPC to come up"
...
def start_nodes(num_nodes, dirname, extra_args=None, rpchost=None, binary=None):
    """
    Start multiple onexds, return RPC connections to them
    """
